...
public class IntegrationControllerOld {
    ...
    public void integrate(IncomingInvoiceVersion invoiceVersionDto) {

    }

    @ResponseBody
    @RequestMapping(path = "digest/integration", method = RequestMethod.POST)
    public void integrateInvoice(@RequestBody IncomingInvoiceVersion invoiceVersionDto) {
...
//                    return Response.ok(Static.getDefaultDto("INCOHERENT.AMOUNT")).build();
    ...
    private List<Invoice> getSubInvoices(IncomingInvoiceVersion parentInvoiceVersionDto, Invoice parentInvoice) {

        final List<Invoice> subInvoices = Collections.emptyList();
        final List<IncomingInvoiceVersion> subInvoiceVersionDtos = parentInvoiceVersionDto.getSubInvoices();
