...
import org.guce.epayment.core.repositories.ReceiptIntervalRepository;
...
public class TransferOrderServiceImpl implements TransferOrderService {
    ...
    @Autowired
    private ReceiptIntervalRepository sittingDateRepository;
    @Autowired
...
//        if (canValidate) {
//            paymentService.updateInvoices(transferOrder, InvoiceStatus.PAID);
//            paymentService.setDecision(transferOrder, PaymentStatus.VALIDATED);
...
//
//                paymentService.updateInvoices(transferOrder, InvoiceStatus.PAYMENT_CANCELED);
//                paymentService.setDecision(transferOrder, PaymentStatus.CANCELED);
//            } else {
//
//                // rejet
...
//
//                paymentService.updateInvoices(transferOrder, InvoiceStatus.PAYMENT_REJECTED);
//                paymentService.setDecision(transferOrder, PaymentStatus.REJECTED);
//            }
//        }
//
...
//        paymentService.update(transferOrder);
    ...
    private boolean canValidate(TransferOrder transferOrder, Step step, Partner partner, int currentLevel) {
        ...
        if (StepCode.T11.equals(step.getCode())) {
        ...
        final String stepCode = step.getCode().name();
...
//        return transferOrder.getBankGateway().equals(transferOrder.getBeneficiaryBank());
    ...
    private void setReceiptDate(TransferOrder transferOrder) {
        List<ReceiptInterval> sittingDates = sittingDateRepository.findAll();
        if (!sittingDates.isEmpty()) {
            transferOrder.setReceiptDate(DateUtils.getTransferOrderReceiptDate(sittingDates.get(0).getMinTime(), sittingDates.get(0).getMaxTime()));
        }
    }
