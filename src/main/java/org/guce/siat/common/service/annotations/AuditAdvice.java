...
public class AuditAdvice implements Serializable
{
	...
	public Object auditAround(final ProceedingJoinPoint proceedingJoinPoint, final Audit auditAnnotation, final Object parameter)
			throws Throwable
	{

		final String operationType = auditAnnotation.operationType();

		if (AuditConstants.SAVE.getCode().equals(operationType))
		{
			return auditAroundForSaveOperation(proceedingJoinPoint, parameter, operationType);
		}
		else if (AuditConstants.UPDATE.getCode().equals(operationType))
		{
			return auditAroundForUpdateOperation(proceedingJoinPoint, parameter, operationType);
		}
		else if (AuditConstants.DELETE.getCode().equals(operationType))
		{
			return auditAroundForDeleteOperation(proceedingJoinPoint, parameter, operationType);
		}
		else if (AuditConstants.BAD_CREDENTIALS.getCode().equals(operationType))
		{
	...
	private Object auditAroundForDeleteOperation(final ProceedingJoinPoint proceedingJoinPoint, final Object model,
			final String operationType) throws BusinessException, Throwable
	{
		Object finalTarget = null;
		finalTarget = proceedingJoinPoint.proceed();
	 ...
	 *            the business exception
	 */
	private void saveAuditLogMessage(final String operationType, final Object model) throws BusinessException, IOException
	{
		final AuditEntity auditEntity = new AuditEntity();
		if (AuditConstants.BAD_CREDENTIALS.getCode().equals(operationType))
		{
		...
		auditEntity.setAction(operationType);
	 ...
	 *            the business exception
	 */
	private void saveAuditListLogMessage(final String operation, final Object model) throws BusinessException
	{
		@SuppressWarnings("unchecked")
		final List<AbstractModel> abstractModelList = (List<AbstractModel>) model;

		final List<AuditEntity> auditEntitiesList = new ArrayList<AuditEntity>();
		...
		for (final AbstractModel abstractModel : abstractModelList)
		{
			...
			auditEntity.setUsername(SecurityUtils.getCurrentUser());
			auditEntity.setAuditDate(new Date());
			auditEntity.setAction(operation);
