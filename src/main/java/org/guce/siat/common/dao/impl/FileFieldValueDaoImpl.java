...
public class FileFieldValueDaoImpl extends AbstractJpaDaoImpl<FileFieldValue> implements FileFieldValueDao {
    ...
    public FileFieldValue findValueByFileFieldAndFile(final String fileFieldCode, FileTypeCode fileTypeCode, String numeroDemande) {
        try {
            TypedQuery<FileFieldValue> query = super.entityManager.createQuery("SELECT f FROM FileFieldValue f WHERE f.primaryKey.fileField.code = :fileFieldCode AND f.primaryKey.file.fileType.code = :fileTypeCode AND f.primaryKey.file.numeroDemande = :numeroDemande ORDER BY f.primaryKey.file.createdDate DESC", FileFieldValue.class);
