...
/**
 * The listener interface for receiving jmsMailMessage events. The class that is
 * interested in processing a jmsMailMessage event implements this interface,
 * and the object created with that class is registered with a component using
 * the component's <code>addJmsMailMessageListener<code> method. When
        ...
        try {
            if (message instanceof MapMessage) {
                ...
                emailSenderService.send(m.getString(MailConstants.SUBJECT), m.getString(MailConstants.FROM),
                        m.getString(MailConstants.VMF), map, new String[]{m.getString(MailConstants.EMAIL)});
