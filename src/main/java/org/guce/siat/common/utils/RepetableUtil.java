...
public final class RepetableUtil {
     ...
     *
     * @param fileItemFieldValue the file item field value
     * @param rowSeparator the row separator
     * @return the string[]
     */
     ...
     *
     * @param repeatablefileFieldValueList2 the repeatablefile field value list2
     * @param row the row
     *
     * @return the list
     */
    public static List<Tab> initDynamicTabs(final List<FileItemFieldValue> repeatablefileFieldValueList2, final Integer row) {
        ...
        if (CollectionUtils.isNotEmpty(repeatablefileFieldValueList2)) {
            for (int i = 0; i < repeatablefileFieldValueList2.size(); i++) {
                ...
                final List<TableColHeader> colHeaders = new ArrayList<>();
                ...
                if (column.length > 1) { // détécter si il y a un bloc répétable
                    // sup
                    tab.setFils(true);
                    final List<String> repetable = new ArrayList<>();
                ...
                System.arraycopy(colunmsFk, 1, arrayValues, 1, colunmsFk.length - 1);
                ...
                if (row != null) {
                    ...
                    final List<Map<Integer, String>> tableContent1 = new ArrayList<>();
                ...
                if (arrayValues != null) {
                    ...
                    if (columns != null) {
                        for (String column1 : columns) {
                            ...
                            tableColHeader.setLabelFr(column1);
                            tableColHeader.setLabelEn(column1);
                ...
                if (colHeaders.size() > 3) {
                    final List<TableColHeader> headers = new ArrayList<>();
    ...
    private static List<Integer> recupererFilsByParent(final Integer row, final FileItemFieldValue fileItemFieldValue) {
        ...
        if (array.length > 0) {
            ...
            for (String rowS1 : rowS) {
                if (StringUtils.isNotBlank(rowS1)) {
                    rows.add(Integer.parseInt(rowS1));
                }
            }
        }
        return rows;
     ...
     *
     * @param nonRepetablefileItemFiledValues the non repetablefile item filed
     * values
     * @param repetablefileItemFiledValues the repetablefile item filed values
     * @param fieldGroups the field groups
     * @param applicationPropretiesService the application propreties service
     * @return the list
     */
    public static List<FieldGroupDto<FileItemFieldValue>> groupFileItemFieldValues(
        ...
        RepetableUtil.applicationPropretiesService = applicationPropretiesService;
        final List<FieldGroupDto<FileItemFieldValue>> fileFieldGroupDtos = new ArrayList<>();
        for (final FieldGroup fieldGroup : fieldGroups) {
            final FieldGroupDto<FileItemFieldValue> fileFieldGroupDto = new FieldGroupDto<>();
            fileFieldGroupDto.setLabelFr(fieldGroup.getLabelFr());
            fileFieldGroupDto.setLabelEn(fieldGroup.getLabelEn());
            final List<FileItemFieldValue> listFileFieldValues = new ArrayList<>();
            final List<Tab> tabs = new ArrayList<>();
            ...
            for (final FileItemFieldValue repFileItemFieldValue : repetablefileItemFiledValues) {
                if (fieldGroup.getId().equals(repFileItemFieldValue.getFileItemField().getGroup().getId())) {
                    final List<FileItemFieldValue> repetables = new ArrayList<>();
                    repetables.add(repFileItemFieldValue);
